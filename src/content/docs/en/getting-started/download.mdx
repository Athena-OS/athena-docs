...
1. Import the GPG key for verifying the authenticity:
   ...
   ```
   gpg: key A3F78B994C2171D5: "xxxxxx <xxx@yy.z>" imported
   gpg: Total number processed: 1
   gpg:               imported: 1  (RSA: 1)
   ```

2. Download the ISO file and its `.sig` signature file and run the following command (by replacing `athenaos-<file-version>-<arch>.iso` with the actual ISO filename):
   ```shell
   ...
   gpg: Good signature from "xxxxxx <xxx@yy.z>" [unknown]
