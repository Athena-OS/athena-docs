...
Athena OS can be installed by an ISO file downloaded from the [official website](https://github.com/Athena-OS/athena-iso/releases/latest) and currently available for **x86_64** architectures.
...
### Verify Athena OS ISO Image Integrity
...
* if you are on Windows, run the following command (by replacing `athena-<file-version>-<arch>.iso` with the actual ISO filename):
  ...
  ```
  Note: be sure to have `certutil` installed on Windows, otherwise you could use other tools such as [Microsoft File Checksum Integrity Verifier](http://www.microsoft.com/en-us/download/details.aspx?id=11533).

and check if the output is equal to the **SHA-256** hash value shown on the [official website](https://github.com/Athena-OS/athena-iso/releases/latest).

Assuming that the official website has not been tampered, if the values are equal, the ISO image is current and trusted.

### Verify Athena OS ISO Image Authenticity
...
1. Import the GPG key for verifying the authenticity:
   ```
   gpg --keyserver hkps://keyserver.ubuntu.com --recv-key A3F78B994C2171D5
   ```
   ...
   ```
   Note: if you get an error, run again the command.
...
2. Download the ISO file and its `.sig` signature file and run the following commmand (by replacing `athena-<file-version>-<arch>.iso` with the actual ISO filename):
   ...
   ```
   If you get a **Good signature** output, the ISO file is authentic:
   ```
