...
Athena OS is also a capable general-purpose operating system. Its performance foundation - BORE scheduler, -O3 optimization, 1000Hz timer, native CPU targeting - means it competes directly with gaming and performance-focused distributions on raw desktop speed. Its security stack - Secure Boot, TPM-bound LUKS, AppArmor, Firejail, USBGuard - exceeds what most mainstream distributions ship by default. A developer, researcher, or power user running Athena as a daily driver gets a faster and better-protected system than they would get from most conventional choices.
...
```
UEFI Firmware
    ↓ validates signature via Secure Boot (MOK enrolled by Aegis)
systemd-boot
...
| **Process** | Firejail sandboxing | Installed; invoke manually per-command | [Link](/en/security/sandboxing) |
...
Athena OS ships with Secure Boot support managed automatically by the Aegis installer. Each kernel is packaged as a **Unified Kernel Image (UKI)**, a single signed EFI executable containing the kernel, initramfs, and kernel command line together. Tampering with any component invalidates the entire cryptographic signature.
...
**Firejail** is installed but provides **no automatic protection**. An application launched normally runs outside any sandbox unless you explicitly wrap it.
...
# Run Burp Suite with access only to a project directory
firejail --private --whitelist=$HOME/burp-projects burpsuite
...
The practical rule: anything that touches untrusted content should run through Firejail. Files received from client systems, samples from engagements, unknown binaries, and documents of any kind are candidates. The overhead of adding `firejail --net=none --private` before a command is seconds. The risk of not doing so when a document is weaponized is significant.
...
- Run unknown tooling inside a VM or Firejail sandbox rather than directly on the host
...
| Firejail for all untrusted file opening | **Strongly recommended** | Any file from a client system should be opened in `--net=none --private` sandbox |
...
- [ ] **AppArmor:** profiles for browsers and document viewers moved to enforce (`aa-enforce /etc/apparmor.d/<profile>`); verify with `aa-status`
...
- [ ] Open any file received from client systems in Firejail `--net=none --private` before examining
