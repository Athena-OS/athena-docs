...
- `/dev/mem` access is filtered on both profiles; on `linux-aegis-hardened` the device is absent entirely, which blocks memory forensics tools that depend on direct hardware memory access
...
- The `linux-aegis-hardened` kernel removes `/dev/mem` entirely and filters I/O port access, eliminating direct hardware memory interfaces
...
| `/dev/mem` access | ◯ filtered (`STRICT_DEVMEM`) | X filtered (`STRICT_DEVMEM` + `IO_STRICT_DEVMEM`) |
