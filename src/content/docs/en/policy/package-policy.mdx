...
Athena OS uses a package management system based on Pacman, which is responsible for handling the installation, removal, and management of software packages on the system. The Arch Linux community maintains the Arch User Repository (AUR), where users can contribute and share packages that are not part of the official Arch Linux repositories.
