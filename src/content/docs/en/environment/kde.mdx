...
Kando cannot directly bind global shortcuts on KDE Wayland that can be used for invoking Kando menu. To set a shortcut to invoke the menu, right-click on Kando icon on bottom-right -> **Show settings** -> in **Shortcut ID** type a meaningful Shortcut ID, for example **Invoke**, then, open **System Settings** of KDE -> find for **Shortcuts** -> find for **Kando** and you will see an empty **INVOKE** entry, click on **Add...** and by your keyboard, press the keys that will be used to invoke Kando menu, for example **CTRL+SPACE**.
