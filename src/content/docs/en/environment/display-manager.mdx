...
![Ly Matrix Theme](@assets/images/ly-matrix.png)
...
Athena OS provides **SDDM** as Display Manager because of its reliability and flexibility, in particular the [sddm-astronaut-theme](https://github.com/Keyitdev/sddm-astronaut-theme).
...
On some systems, the display manager may appear on an unexpected monitor.
In framebuffer-based setups, this can be influenced via kernel parameters.
...
If this does not work, monitor selection must be handled via GPU driver
configuration or display manager–specific settings.
