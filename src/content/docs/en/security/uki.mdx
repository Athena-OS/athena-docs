...
To modify kernel parameters, edit **/etc/kernel/cmdline**. Once the file is saved, run:
```shell
sudo rebuild-ukis
```
to rebuild UKI according to the new kernel parameters.

This command is automatically run each time kernel or microcode are updated.
