...
To improve integrity checks in Athena at firmware, boot and kernel level, after Athena OS installation, set up the PCRs you wish, for example PCR7, PCR11 and PCR15, by running:
```
sudo systemd-cryptenroll --tpm2-device=auto --tpm2-pcrs=7+11+15 /dev/mapper/nvme0n1p5crypted
```
and providing the LUKS password.

:::note
The device to specify must be the LUKS decrypted one.
:::
...
If you are interested to set different PCRs, refer to the table above for details and assign their ID number to the `--tpm2-pcrs` parameter.
...
```shell
sudo systemd-cryptenroll --tpm2-device=auto --tpm2-pcrs=7+9+11+14 /dev/mapper/nvme0n1p5crypted
```
