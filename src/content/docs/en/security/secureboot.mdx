...
:::note
If the **Enroll MOK** entry is not shown, select **Enroll key from disk** and search for **MOK.cer** file inside the directory with long string and, inside of it, enter into **EFI** directory and select the Athena Secure Boot **.cer** file.
:::
