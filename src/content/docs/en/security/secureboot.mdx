...
Athena Arch supports Secure Boot in UEFI. Its deployment is managed automatically by Dory installer that will detect if the system is UEFI and will create the proper certificates and sign the boot and kernel files needed for this process.
...
At first reboot, it gets a **MOK (Machine Owner Key)** request for enrolling the Secure Boot certificate previously created for you by Dory.
...
```shell
sudo rm -rf /efi/EFI/Athena/AthenaSecureBoot.cer
```
...
sbsign --key /etc/secureboot/keys/MOK.key --cert /etc/secureboot/keys/MOK.crt --output /efi/EFI/systemd/systemd-bootx64.efi /efi/EFI/systemd/systemd-bootx64.efi
cp /efi/EFI/systemd/systemd-bootx64.efi /efi/EFI/BOOT/grubx64.efi

ukify build --linux /boot/vmlinuz-linux-lts --initrd /boot/initramfs-linux-lts.img --cmdline <list of kernel params> --os-release /usr/lib/os-release --uname linux-lts --signtool=sbsign --secureboot-private-key /etc/secureboot/keys/MOK.key --secureboot-certificate /etc/secureboot/keys/MOK.crt --output /efi/EFI/Athena/linux-lts.efi

ukify build --linux /boot/vmlinuz-linux-hardened --initrd /boot/initramfs-linux-hardened.img --cmdline <list of kernel params> --os-release /usr/lib/os-release --uname linux-hardened --signtool=sbsign --secureboot-private-key /etc/secureboot/keys/MOK.key --secureboot-certificate /etc/secureboot/keys/MOK.crt --output /efi/EFI/Athena/linux-hardened.efi
```
