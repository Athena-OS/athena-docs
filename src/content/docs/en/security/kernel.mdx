...
- `/dev/mem` device absent entirely (`CONFIG_DEVMEM` not set)
...
| `CONFIG_IO_STRICT_DEVMEM` | enabled | Filters I/O port access - defense-in-depth alongside `CONFIG_DEVMEM` not being set; ensures even if `/dev/mem` were present, I/O memory regions claimed by drivers cannot be accessed. |
...
| Lockdown integrity + `CONFIG_DEVMEM` not set | `/dev/mem` absent entirely; kexec disabled | Live memory acquisition: use LiME with signed module |
