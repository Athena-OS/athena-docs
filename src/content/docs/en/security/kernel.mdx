---
title: Pentesting-patched Kernel
...
- kptr fully restricted (`kptr_restrict=2` default) - kernel pointers hidden from all users including root
- `DEBUG_FS_ALLOW_NONE` - debugfs inaccessible by default; eliminates a significant kernel introspection surface
- `device_sidechannel_restrict=1` - character and block device timestamps normalized for unprivileged users, preventing timing side-channel attacks
...
- `SLAB_MERGE_DEFAULT` disabled - prevents slab cache merging, reducing heap spray effectiveness
- `RANDOM_KMALLOC_CACHES` - randomizes which slab cache kmalloc allocations use
- `BUG_ON_DATA_CORRUPTION` - kernel panics on detected memory corruption rather than continuing; automatically enables `LIST_HARDENED`
- `KFENCE_BUG_ON_DATA_CORRUPTION` - kernel panics on kfence-detected memory corruption
...
- `PANIC_ON_OOPS` - kernel panics on oops rather than attempting to continue in a potentially corrupted state
...
- `RANDOMIZE_KSTACK_OFFSET_DEFAULT` - kernel stack offset randomized per syscall, defeating stack-based info leaks
- `ARCH_MMAP_RND_BITS` set to maximum - strongest available ASLR entropy for mmap base
- `ARCH_MMAP_RND_COMPAT_BITS` set to maximum - same for compat applications
- `COMPAT_BRK` disabled - heap randomization enabled
- `DEFAULT_MMAP_MIN_ADDR=65536` - raised from 4096, reducing NULL pointer dereference attack surface
...
- `IO_STRICT_DEVMEM` enabled - even root cannot access I/O memory regions actively claimed by a driver via `/dev/mem`
...
- `io_uring` disabled by default - io_uring has been a prolific kernel exploit surface; disabled unless explicitly re-enabled via sysctl
...
| `CONFIG_IO_STRICT_DEVMEM` | enabled | Filters I/O port access through `/dev/mem` - root cannot access I/O memory regions actively claimed by a driver, blocking hardware-level attacks. |
