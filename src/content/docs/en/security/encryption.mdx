...
This integration is handled automatically by **Dory Installer**, which can detect your TPM device and configure LUKS to unlock the encrypted partition using the TPM key at boot time.
...
During installation by **Dory**, you can choose to:
- **Encrypt ROOT (LUKS)** - a passphrase is requested.
- **Bind the key to TPM** - if TPM is detected, Dory will store the decryption key securely in the TPM and configure automatic unlocking.
...
```shell
sudo systemd-cryptenroll --tpm2-device=auto --tpm2-public-key=/etc/systemd/tpm2-pcr-initrd-public-key.pem --tpm2-public-key-pcrs=11 --tpm2-pcrs=7+15 /dev/nvme0n1p3
```
