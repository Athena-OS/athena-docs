...
:::caution
Some packages could disable NVIDIA modules (i.e., bumblebee). For checking if NVIDIA modules are disabled, give a look to the files in `/etc/sysctl.d/` directory. 
:::

## Integrated GPU and Dedicated GPU

During the installation phase in Calamares, Athena will set the NVIDIA drivers **automatically**. The switch between **Integrated** and **Dedicated GPU** can be managed by [EnvyControl](https://github.com/bayasdev/envycontrol).
