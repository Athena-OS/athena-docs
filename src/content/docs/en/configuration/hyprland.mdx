...
Note that Hyprland is currently in beta. It doesn't work in virtual machines without **GPU passthrough**, so it is suggested to use it on bare metal.
...
Edit the lines that say timeout=NUMBER. The number is in seconds. For example, if you want your device to sleep after 15 minutes, the `listener` entry for it should look like this:
```ini
listener {
    timeout = 540 # 9mins
...
* **~/.local/bin**: Contains local scripts and possibly binaries
...
- To change the wallpaper, hit **Super** to open the launcher and type in `>img`. A popup will show up, allowing to select an image.
  - Colors will automatically generate for the chosed wallpaper
  - For more commands, see the cheat sheet **(Super + /)**

## Keybindings

To view them any time, hit **Super + /**.
