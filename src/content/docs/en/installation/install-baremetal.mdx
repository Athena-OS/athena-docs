...
In case you are installing Athena OS in dual boot with Windows, make sure that **Fast Startup** is disabled. As explained [here](https://wiki.archlinux.org/title/Dual_boot_with_Windows#Fast_Startup_and_hibernation), data loss or filesystem corruption can occur when dual-booting between Windows and another operating system, like Linux, if either OS hibernates instead of fully shutting down. For example, if you access to Linux after Windows hibernated the system, the WiFi module could stop to work.

To disable Fast Startup on Windows, open **Control Panel** -> **Hardware and Sound** -> **Power Options** -> click on **Choose what the power buttons do** on the left side -> click on **Change Settings that are currently unavailable** on top (This will ask for administrator permissions) -> Unselect the **Turn on fast startup** option and click on **Save Changes** button.
...
Finally, run the **Dory Installer**.

[Dory](https://github.com/Athena-OS/dory) is a terminal-based installer, developed, tailored and optimized specifically for Athena OS.

To provide more accessible resources to the users, Dory provides a terminal interface to configure and install your systems according to your preferences.

Dory will improve over time in order to detect automatically your hardware. It means that the installer is able to detect your environment (baremetal or virtualized), your CPU and your GPU. According to this information, all the needed drivers are automatically installed with no any actions by your side.

Dory can be run by running `sudo dory` command and requires a working Internet connection.
...
The installation logs will be saved to `/tmp/dory.log` and copied to the installed system to `/var/log/dory.log`.

Open the terminal and run:
```shell
sudo dory
```
![Dory](@assets/images/dory.png)

Choose your favourite base system
![Dory Base](@assets/images/dory-base.png)
...
![System Timezone](@assets/images/dory-timezone.png)

## Keyboard Layout

Select the keyboard layout

![Keymap](@assets/images/dory-keyboard.png)
...
![Dory Device List](@assets/images/dory-device-list.png)

Once you select one, you are asked for a swap device.

![Dory Swap](@assets/images/dory-swap.png)
...
![Dory Filesystem](@assets/images/dory-filesystem.png)
...
![Dory Encrypt](@assets/images/dory-encrypt-root.png)

If you select **Encrypt ROOT (LUKS)**, and press **Continue**, insert your LUKS passphrase.

![Dory LUKS](@assets/images/dory-encrypt-luks.png)
...
![Dory Partition List](@assets/images/dory-partition-manual-list.png)
...
![Dory Partition Mark](@assets/images/dory-partition-mark.png)

Press on **Mark For Modification** entry.

![Dory Partition Mark](@assets/images/dory-partition-manual-modify.png)
...
![System Hostname](@assets/images/dory-hostname.png)

## Root password

Set the password for **root** account

![User Password](@assets/images/dory-root.png)

## Users

Set your user account
![Users](@assets/images/dory-users.png)

By pressing on **Add a new user** you can setup your account.

![Users](@assets/images/dory-user-info.png)

## Desktop

Select your favourite environment
![System Desktop](@assets/images/dory-desktop.png)

## Login Manager

Select your [favourite display manager](https://athenaos.org/en/configuration/display-manager/) for login.

![Display Manager](@assets/images/dory-dm.png)

## Design

Select your favourite design

![System Theme](@assets/images/dory-design.png)
...
![Install Configuration](@assets/images/dory-config-preview.png)
 
Finally, if you are ready to install, proceed on **Begin Installation**.

![Install System](@assets/images/dory-installation.png)
 
During the installation, installation logs are saved in `/tmp/dory.log`.

If the installation fails, you can generate a link with install logs that can be used for troubleshooting.

![Install Fails](@assets/images/dory-failed.png)
![Install Logs](@assets/images/dory-logs.png)
