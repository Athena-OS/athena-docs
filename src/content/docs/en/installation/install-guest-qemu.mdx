...
---
ort ThemedImage from '@components/ThemedImage.astro';
...
First, you need to prepare your environment for running Virt-Manager (LibVirt GUI client) that will allow you to run Athena OS in QEMU. In an Arch-based Linux environment, install Virt-Manager by:
```
...
![Virt-Manager Connecting to QEMU/KVM](@assets/virtmanager-connect.png)
...
![Virt-Manager VM Creation](@assets/virtmanager-create.png)
...
![Virt-Manager Locate ISO](@assets/virtmanager-locate.png)

Deselect **Automatically detect from the installation media / source** checkbox and the current step should appear as
![Virt-Manager Set Media](@assets/virtmanager-media.png)

Then, set your resources as CPU and RAM

![Virt-Manager Set Resources](@assets/virtmanager-resources.png)

Set the disk size of the virtual machine

![Virt-Manager Set Disk Size](@assets/virtmanager-disk.png)
...
![Virt-Manager VM Summary](@assets/virtmanager-summary.png)

You could get a message related to the activation of the Virtual Network, press **Yes**

![Virt-Manager Virtual Network](@assets/virtmanager-network.png)
...
![Virt-Manager VM CPU Settings](@assets/virtmanager-processor.png)

Move to **Display Spice** tab, set **Listen type** to **None** and check **OpenGL** in order to use it

![Virt-Manager VM Display Settings](@assets/virtmanager-display.png)

Finally, make sure that the 3D Acceleration is enabled on **Video Virtio** tab

![Virt-Manager VM 3D Acceleration](@assets/virtmanager-accel.png)

At the end, apply all the changes and click on **Begin Installation** button on the top-left side of the settings window.

When the VM is run, install Athena OS as explained in [Installing as single boot](/en/installation/installing-single-boot/) chapter.

*Note: Athena OS automatically recognizes if you are using a virtualized environment so you don't need to install QEMU Guest Agent because the system will do automatically for you during the OS installation.*
...
```
By this command, QEMU will boot the Athena OS ISO that we use for installing Athena OS as explained in [Installing as single boot](/en/installation/installing-single-boot/) chapter.
