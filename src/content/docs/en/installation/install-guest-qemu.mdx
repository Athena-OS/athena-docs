...
```shell
sudo pacman -S virt-manager qemu-full dnsmasq swtpm
...
Afterwards, the VM settings window is opened. On **Overview** pane, in the **Firmware** field select **UEFI**

![Virt-Manager UEFI](@assets/images/virtmanager-uefi.png)

Then, move to **CPUs** tab and expand **Topology** section and increment **Sockets** and **Cores** at least to 2
...
Finally, if your computer supports TPM 2.0, click on **Add Hardware** on bottom-left side, find for **TPM** and add it according to the following image

![Virt-Manager TPM](@assets/images/virtmanager-tpm.png)

At the end, apply all the changes and click on **Begin Installation** button on the top-left side of the settings window.

When the VM is run, install Athena OS as explained in [Installing as baremetal](/en/installation/install-baremetal/) chapter.
...
```
By this command, QEMU will boot the Athena OS ISO that we use for installing Athena OS as explained in [Installing as baremetal](/en/installation/install-baremetal/) chapter.
