...
:::tip
Since you are in a UEFI system, and **you are going to encrypt the root partition**, you need to create an additional partition of **"1024MiB"** with **/boot** as mount point with no flags and **ext4** as filesystem. It is needed to store the initramfs and boot configuration files in an unencrypted partition to make LUKS encryption working properly for root partition.
:::
