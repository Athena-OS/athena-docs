...
```
:::note
If you are using BIOS-Legacy instead of GRUB, edit `bootloader = "systemd";` to `bootloader = "grub";` in `/etc/nixos/configuration.nix`, then edit `/etc/nixos/modules/boot/grub/default.nix` and replace `sda` by your actual disk device. To list your disk devices, run `lsblk -d` (check `TYPE` as `disk`).
:::
