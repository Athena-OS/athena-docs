...
Another method that should work in case you cannot go further over a step, you can run:
```
bcdboot C:\Windows /s C:
```
It will create a second bootmgr entry in Windows that you can set at high priority when you need to access in Windows. It is not a clean solution because probably in this state you could not be able to run `bcdedit` command on Windows due to `The requested system device cannot be found` error. It could require additional steps to fix it.
