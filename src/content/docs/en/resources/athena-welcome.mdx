...
You can choose between **Easy Installation (Offline)** and **Advanced Installation (Online)**. The former does not require an Internet connection but it installs all default elements in Athena; the latter requires an Internet connection but you can customize your final environment.

The details about the installation process are described in [Calamares Installer](/en/configuration/calamares/).
...
**Update Calamares Installer** will update the installer and the binaries it uses to the latest version. **Update All Mirrors** will find the fastest mirrors for your connection in order to speed up the retrieve of the packages to install.

## Running Environment

Once Athena OS is installed, **Athena Welcome** application is automatically run.

![Athena Welcome Running](@assets/images/athena-welcome-running.png)
...
Once selected, click on **Set your Role** button for installing all the tools and resources related to the chosen role.
