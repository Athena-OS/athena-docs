...
```
Assume the bad one is `Boot0002*` entry. In this example with two separated disks, you can see that the assumed good Windows Boot Manager is on HD 1, and the bad one is in HD 2. These entries in general appear because each partition where they are stored maintain a `Boot` folder containing all Windows boot files. It means that a partition of the disk related to HD 2 has a folder named `Boot` that needs to be deleted. In general, when you are not sure in which partition this additional `Boot` folder could be located, mount each partition and check its content. Of course, be careful to not delete the `Boot` folder of the working Windows Boot Manager entry.
:::
