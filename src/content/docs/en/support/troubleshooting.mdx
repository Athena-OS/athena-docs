...
```
Assume the bad one is `Boot0002*` entry. In this example with two separated disks, you can see that the assumed good Windows Boot Manager is on HD 1, and the bad one is in HD 2. These entries in general appear because each partition where they are stored maintain boot files and directories like `Boot`. `bootmgr` and other elements. It means that a partition of the disk related to HD 2 has these boot folder and files that you must delete. In general, when you are not sure in which partition these additional boot folders and files could be located, mount each partition and check its content. Of course, be careful to not delete these boot files inside the partition maintaining the working Windows Boot Manager entry.
:::
