...
The purpose of this guide is to set a mirror for Athena OS ISO images. In Athena OS there is no need to mirror packages because package mirror system is not used on Nix environment.
...
It is important to make the mirror server available to the public so that users can retrieve ISO images. It is possible to reach this purpose by making it a HTTP/HTTPS or FTP server.
