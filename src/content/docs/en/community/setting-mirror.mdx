...
Final consideration: currently the PULL approach by SSH is not a good option (unless hardening the source server). For this reason, the documentation will be updated by using PUSH approach or pure rsync protocol for mirroring.