...
:::caution
If your submitted tool has some dependencies that are not in Athena or Arch Linux repositories, you need to create a PKGBUILD dedicated to it in Athena repository itself. For example, in your forked repository you should create the PKGBUILD file in another directory as `athena/packages/pentesting/<yourdependency>/PKGBUILD`. The PKGBUILD related to a dependency of a tool does not need of the field `groups=('athena' MORE_ATHENA_GROUPS_HERE).`
:::
